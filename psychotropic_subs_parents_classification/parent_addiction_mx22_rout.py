...
query = """
## FRAGE:
Drogensucht, psychotropische Substanzen Sucht bei den Eltern oder deren Partnern beinhalten Drogen Suchtberatung, Drogen Suchterkrankung, Methadon Abgabe, Drogen Suchthilfe, Aufsuchen der Drogen Suchtberatungsstellen, Cannabis/Marihuana/Heroin Konsum, Spielsucht, Drogenentzug, Drogentests, Drogenabstinenz, Drogentherapie, Drogensucht Rückfälle.

Gibt es klare, explizite Hinweise für Drogensucht, psychotropische Substanzen Sucht bei den Eltern oder deren Partnern, aktuell oder in der Vergangenheit, auch wenn die Probleme überwunden sind? 

A: Klare Hinweise vorhanden für Drogensucht, psychotropische Substanzen Sucht bei den Eltern oder deren Partnern
B: Klare Hinweise nicht vorhanden für Drogensucht, psychotropische Substanzen Sucht bei den Eltern oder deren Partnern

Falls vorhanden, liste klare, explizite Hinweise für Drogensucht, psychotropische Substanzen Sucht bei den Eltern oder deren Partnern.
"""
...
def get_answer(text):
    with open('mx22'+iter+'rb.txt', 'w') as file:
        file.write(text)

    loader = TextLoader('mx22'+iter+"rb.txt")
...
sample_df.to_excel(iter+'sample_dfTemp7.xlsx')
sample_df0= pd.read_excel(iter+'sample_dfTemp7.xlsx', sheet_name='Sheet1')
sample_df0['extracted_context'] = sample_df0['context'].apply(lambda x: extract_context(x))
...
file_path = iter+'sample_dfTemp7.xlsx'
...
tokenizer = AutoTokenizer.from_pretrained("suchteltern_class_mx22", do_lower_case = True,
...
model = AutoModelForSequenceClassification.from_pretrained("suchteltern_class_mx22").to("cuda")
