...
sample_path = os.path.expanduser('rbs_eltern1500-2000_psych_codiert_validierung.xlsx')
sample_df1 = pd.read_excel(sample_path, sheet_name='Sheet1')
sample_df1 = sample_df1[['antwort', 'validiert']]
sample_df1 = sample_df1.rename(columns={'antwort': 'sentence', 'validiert': 'label'})

sample_path = os.path.expanduser('rbs_eltern4000-4500_psych_codiert_validierung.xlsx')
sample_df2 = pd.read_excel(sample_path, sheet_name='Sheet1')
sample_df2 = sample_df2[['antwort', 'validiert']]
sample_df2 = sample_df2.rename(columns={'antwort': 'sentence', 'validiert': 'label'})

sample_path = os.path.expanduser('rbs_psych_eltern_sample_antwort_recursFaiss_annotiert.xlsx')
sample_df = pd.read_excel(sample_path, sheet_name='Sheet1')

sample_df = sample_df[sample_df['code'] != -1]
sample_df1 = sample_df1[sample_df1['label'] != -1]
sample_df2 = sample_df2[sample_df2['label'] != -1]
...
# sample_df['antwort']=query + "\nAntwort:\n" + sample_df['antwort']
# print(sample_df['antwort'].head(5))

sample_df = sample_df[['antwort', 'code']]
sample_df = sample_df.rename(columns={'antwort': 'sentence', 'code': 'label'})

sample_df = pd.concat([sample_df, sample_df1, sample_df2])

# Drop empty label values
sample_df = sample_df.dropna(subset=['label'])
...
# sample_path = os.path.expanduser('rbs_psych_eltern_sample_antwort_annotiert.xlsx')
# sample_df = pd.read_excel(sample_path, sheet_name='Sheet1')

# sample_df = sample_df[['antwort', 'code']]
# sample_df = sample_df.rename(columns={'antwort': 'sentence', 'code': 'label'})

# sample_path = os.path.expanduser('validierung_psych_eltern_sample_validiert.xlsx')
# test_df = pd.read_excel(sample_path, sheet_name='Sheet1')

# test_df = test_df[['antwort', 'validierung']]
# test_df = test_df.rename(columns={'antwort': 'sentence', 'validierung': 'label'})
 
# #concat the two dataframes
# sample_df = pd.concat([sample_df, test_df])
# sample_df = sample_df.reset_index(drop=True)
...
psych_eltern_string = [ "psychische Störungen/Erkankungen bei Eltern nicht erwähnt", "psychische Störungen/Erkankungen bei Eltern erwähnt"]
...
id2label={
    ...
    "0": "psychische Störungen/Erkankungen bei Eltern nicht erwähnt",
    "1": "psychische Störungen/Erkankungen bei Eltern erwähnt",    
  }

label2id={
    
    "psychische Störungen/Erkankungen bei Eltern nicht erwähnt": 0,
    "psychische Störungen/Erkankungen bei Eltern erwähnt": 1,    
  }, 
...
folder_path = "results_gelectra_psych_finetune"
try:
    shutil.rmtree(folder_path)
except:
    pass

training_args = TrainingArguments(output_dir='results_gelectra_psych_finetune', 
                                  num_train_epochs=12, 
                                #   logging_steps=100, 
                                #   save_steps=100,
...
# model.save_pretrained("psych_prompt_class_gelectra400")
model.save_pretrained("psych_class_gelectra_rag_recursFaiss2", safe_serialization=True)
tokenizer.save_pretrained("psych_class_gelectra_rag_recursFaiss2")
...
#save tokenizer
# tokenizer.save_pretrained("psych_prompt_class_gelectra400", safe_serialization=True)
