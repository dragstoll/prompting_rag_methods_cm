...
# rbs_psych_eltern_sample.to_pickle(target1)
# rbs_psych_eltern_sample.to_excel(target2, index=False)
# sys.exit()
#sample 30 rows
# rbs_psych_eltern_sample=rbs_psych_eltern_sample.sample(50).reset_index(drop=True).copy()
...
query = "Liste klare, explizite Hinweise für Drogenkonsum oder Drogensucht bei den Eltern oder deren Partner, auch in der Vergangenheit? "
...
tokenizer = AutoTokenizer.from_pretrained("sucht_class_gelectra_rag2", do_lower_case = True,
                                         use_fast=True,
...
model = AutoModelForSequenceClassification.from_pretrained("sucht_class_gelectra_rag2").to("cuda")
